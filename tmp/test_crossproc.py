...
tmp = tempfile.NamedTemporaryFile(
    suffix=".html", delete=False, dir="/home/yichuan/pixelrag/tmp"
)
...
async def main():
    ...
    for i in range(3):
        for j in range(i + 1, 3):
            a = np.array(
                Image.open(io.BytesIO(results[i][1])).convert("RGB"), dtype=np.float32
            )
            b = np.array(
                Image.open(io.BytesIO(results[j][1])).convert("RGB"), dtype=np.float32
            )
            if a.shape != b.shape:
                print(
                    f"proc{i} vs proc{j}: SHAPE MISMATCH {a.shape} vs {b.shape}"
            ...
            print(
                f"proc{i} vs proc{j}: mean_diff={mean_d:.2f}  "
