import sys, os, asyncio, json, subprocess, signal, time, io, base64
...
async def render(port, url):
    proc = subprocess.Popen(
        [CHROME, f"--remote-debugging-port={port}", "--headless",
    ...
    for name in ["Vanvaas", "Noel_Williams_(Northern_Ireland_politician)"]:
        ...
        for i in range(3):
            for j in range(i + 1, 3):
                a = np.array(Image.open(io.BytesIO(tiles[i])).convert("RGB"),
                             dtype=np.float32)
                b = np.array(Image.open(io.BytesIO(tiles[j])).convert("RGB"),
                             dtype=np.float32)
                if a.shape != b.shape:
                ...
                diff = np.abs(a - b)
                lines.append(
                    f"  {i}v{j}: mean={diff.mean():.4f} max={diff.max():.0f}")
        lines.append("")
