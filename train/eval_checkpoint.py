...
def main():
    ...
    # Load model
    model = BiQwen3.from_pretrained("Qwen/Qwen3-VL-Embedding-2B", dtype=torch.bfloat16).to(device)
    ...
    ppt = processor.image_processor.patch_size ** 2 * processor.image_processor.merge_size ** 2
    processor.image_processor.max_pixels = args.max_num_visual_tokens * ppt
    processor.image_processor.min_pixels = max(processor.image_processor.min_pixels, ppt)
    ...
    doc_paths = sorted([os.path.join(tiles_dir, f) for f in os.listdir(tiles_dir) if f.endswith(".png")])
    test_data = {
    ...
    output_path = None
    if args.checkpoint != "base":
        ckpt_name = os.path.basename(os.path.dirname(args.checkpoint)) + "_" + os.path.basename(args.checkpoint)
    else:
        ckpt_name = "base"
    ...
    metrics = tc.run_miniv6_eval(
        model, processor, test_data, device,
