...
def build(config: dict, limit: int | None = None, force: bool = False) -> Path:
    ...
    for doc in docs:
        idx = len(articles)
        articles.append({"id": str(doc.id), "url": doc.url, "path": doc.path, "metadata": doc.metadata or {}})
        if doc.url:
    ...
    if url_docs:
        new_url_docs = [(idx, d) for idx, d in url_docs
        ...
        if new_url_docs:
            ...
            render_urls(urls, str(tiles_dir), backend=backend, stems=stems, **ingest_cfg)
        skipped = len(url_docs) - len(new_url_docs)
        logger.info("  Rendered %d URLs (%d skipped, already exist)", len(new_url_docs), skipped)
    ...
    logger.info("  Saved %d article mappings to %s", len(article_entries), articles_path)
    ...
    if device == "cpu":
        ...
        cmd = [
            sys.executable, "-m", "pixelrag_embed.embed_cpu",
            "--shard-dir", str(tiles_dir),
    ...
    total_vectors = sum(np.load(f, mmap_mode="r")["embeddings"].shape[0] for f in npz_files)
    nlist = min(4096, max(1, total_vectors // 40))
    logger.info("Stage 4/4: Building FAISS index (%d vectors, nlist=%d)...", total_vectors, nlist)
    subprocess.run(
        [sys.executable, "-m", "pixelrag_embed.index", "build",
...
def main():
    ...
    parser.add_argument("--source-type", default=None, help="Source type (kiwix/web/pdf/local)")
    parser.add_argument("--output", "-o", default=None, help="Output directory")
    parser.add_argument("--device", default=None, choices=["cpu", "cuda"], help="Embedding device")
    parser.add_argument("--limit", "-n", type=int, default=None, help="Max documents to process")
    parser.add_argument("--force", "-f", action="store_true", help="Clean output and rebuild from scratch")
